...
static jit_integer_range_t jit_word_range(sv_op_t op, jit_integer_range_t left, jit_integer_range_t right) {
...
static void jit_emit_integer_constant(
  ...
  MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_MOV,
    MIR_new_reg_op(ctx, dst), MIR_new_uint_op(ctx, (uint64_t)range.min)));
...
static bool jit_emit_integer_arithmetic(
  ...
  if (op == OP_ADD || op == OP_ADD_NUM) {
    code = MIR_ADD; lo = (__int128)l.min + r.min; hi = (__int128)l.max + r.max;
  ...
  MIR_append_insn(ctx, fn, MIR_new_insn(ctx, code,
    MIR_new_reg_op(ctx, dst), MIR_new_reg_op(ctx, left), MIR_new_reg_op(ctx, right)));
  vs->slot_type[vs->sp - 1] = SLOT_I32;
...
static void mir_emit_decode_ref(
...
static void mir_emit_cage_offset(
...
static void mir_d_to_i64_non_nan(MIR_context_t ctx, MIR_item_t fn,
...
static void mir_emit_slot_boxed(MIR_context_t ctx, MIR_item_t fn,
...
static void vstack_ensure_num(jit_vstack_t *vs, int idx,
  ...
  else mir_i64_to_d(ctx, fn, vs->d_regs[idx], vs->regs[idx], d_slot);
  vs->slot_type[idx] = SLOT_NUM;
}

static bool vstack_prepare_num(jit_vstack_t *vs, int idx,
                               MIR_context_t ctx, MIR_item_t fn,
                               MIR_reg_t d_slot) {
...
static void vstack_rebox_binop_operands(
...
static void mir_emit_self_binding_guard(
  ...
  uint64_t func_tag = NANBOX_PREFIX
    | ((ant_value_t)(kTypeFunction & NANBOX_TYPE_MASK) << NANBOX_TYPE_SHIFT);
...
static int mir_next_reg_site(int *next_site) {
  ANT_ASSERT(next_site != NULL, "missing MIR register site counter");
  return (*next_site)++;
}

static MIR_reg_t mir_emit_exact_integer_guard(
  MIR_context_t ctx, MIR_item_t fn,
...
static MIR_reg_t mir_emit_array_index_guard(
  MIR_context_t ctx, MIR_item_t fn,
  MIR_reg_t boxed, MIR_reg_t known_double, bool is_known_double,
  MIR_reg_t d_slot, MIR_label_t slow, int site
) {
...
static MIR_reg_t mir_emit_known_array_index_guard(
  ...
  if (!integer) {
    ...
    mir_emit_is_num_guard(ctx, fn, 0, boxed, slow);
    MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_BEQ,
      MIR_new_label_op(ctx, hit), MIR_new_reg_op(ctx, boxed),
      MIR_new_reg_op(ctx, cached_key)));
    MIR_reg_t checked = mir_emit_array_index_guard(
      ctx, fn, boxed, 0, false, d_slot, slow, site);
    MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_MOV,
      MIR_new_reg_op(ctx, cached_key), MIR_new_reg_op(ctx, boxed)));
    MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_MOV,
    ...
    MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_MOV,
      MIR_new_reg_op(ctx, result), MIR_new_reg_op(ctx, cached_index)));
    return result;
  }
  MIR_append_insn(ctx, fn, MIR_new_insn(ctx, MIR_UBGE,
    MIR_new_label_op(ctx, slow), MIR_new_reg_op(ctx, integer),
    MIR_new_uint_op(ctx, UINT32_MAX)));
  return integer;
}

static MIR_reg_t mir_emit_dense_numeric_element_guard(
  MIR_context_t ctx, MIR_item_t fn,
...
static void mir_emit_word32_binary(
  ...
  switch (op) {
    case OP_BAND: operation = MIR_ANDS; break;
    case OP_BOR:  operation = MIR_ORS;  break;
    case OP_BXOR: operation = MIR_XORS; break;
    case OP_SHL:  operation = MIR_LSHS; break;
    case OP_SHR:  operation = MIR_RSHS; break;
    default:      operation = MIR_URSHS; break;
  }
...
static void mir_emit_numeric_equality(
  MIR_context_t ctx, MIR_item_t fn,
  MIR_reg_t left, MIR_reg_t right, MIR_reg_t result,
...
static void mir_emit_strict_tagged_equality(
  MIR_context_t ctx, MIR_item_t fn,
  MIR_reg_t left, MIR_reg_t right, MIR_reg_t result,
  MIR_reg_t scratch, bool invert,
...
static void mir_emit_ic_obj_epoch_guard(
  MIR_context_t ctx, MIR_item_t fn,
...
static bool jit_inlineable(sv_func_t *f) {
  ...
  while (ip < end) {
    ...
    if (
      ...
      sv_get_u16(ip + 2) > SV_JIT_ARGS_BUF_CAP
    ) return false;
...
#define INL_FLUSH_ALL() do {                                            \
  ...
  while (ip < end) {
    ...
    switch (op) {
      ...
      case OP_UNDEF: mir_load_imm(ctx, jit_func, inl_vs[isp++], mkval(kTypeUndefined, 0)); break;
      case OP_NULL:  mir_load_imm(ctx, jit_func, inl_vs[isp++], mkval(kTypeNull, 0));  break;
      ...
      case OP_SEQ: case OP_SNE: case OP_EQ: case OP_NE: {
        uint8_t feedback = sv_func_type_feedback(callee)
          ? sv_func_type_feedback(callee)[inl_bc_off] : 0;
        if (sv_tfb_specialization_ready(feedback) &&
          ...
          MIR_item_t helper = op == OP_SEQ ? imp_seq
            : op == OP_EQ ? imp_eq : op == OP_NE ? imp_ne : imp_sne;
          if (inl_num[left_idx])
            mir_d_to_i64(
      ...
      case OP_IS_UNDEF: case OP_IS_NULL: {
        ...
        uint64_t cmp_val = (op == OP_IS_UNDEF)
          ? mkval(kTypeUndefined, 0) : mkval(kTypeNull, 0);
...
sv_jit_func_t sv_jit_compile(ant_t *js, sv_func_t *func, sv_closure_t *hint_closure) {
  ...
  MIR_item_t regexp_exec_truthy_proto = MIR_new_proto(
    ctx, "regexp_exec_truthy_proto",
    1, &call_ret,
    5,
    MIR_T_I64, "vm",
    MIR_T_I64, "js_p",
  ...
  MIR_item_t shape_transition_proto = MIR_new_proto(ctx, "shape_transition_proto",
    0, NULL, 2,
    MIR_T_P, "obj",
    MIR_T_P, "to_shape");
  ...
  MIR_item_t regexp_proto = MIR_new_proto(ctx, "regexp_proto",
    1, &regexp_ret, 4,
    MIR_T_I64, "vm",
    MIR_T_I64, "js",
    MIR_JSVAL, "pattern",
  ...
  MIR_item_t strict_arguments_proto = MIR_new_proto(
    ctx, "strict_arguments_proto",
    1, &special_obj_ret,
    4,
    MIR_T_I64, "vm",
    MIR_T_I64, "js",
  ...
  MIR_item_t iter_next_proto = MIR_new_proto(ctx, "inext_proto",
    1, &iter_next_ret, 4,
    MIR_T_I64, "vm",
    MIR_T_I64, "js",
    MIR_T_P,   "iter_buf",
  ...
  MIR_item_t imp_strict_arguments =
    MIR_new_import(ctx, "jit_helper_strict_arguments");
  MIR_item_t imp_for_of      = MIR_new_import(ctx, "jit_helper_for_of");
  MIR_item_t imp_iter_next   = MIR_new_import(ctx, "jit_helper_iter_next");
  ...
  MIR_item_t imp_shape_transition =
    MIR_new_import(ctx, "jit_helper_shape_transition");
  ...
  MIR_reg_t r_cage_base = MIR_new_func_reg(
    ctx, jit_func->u.func, MIR_T_I64, "cage_base");
  mir_load_imm(ctx, jit_func, r_cage_base, (uint64_t)ant_cage_base());
  ...
  vs.integer_range = calloc((size_t)vs.max, sizeof(jit_integer_range_t));

  if (!vs.regs || !vs.known_func || !vs.d_regs || !vs.slot_type || !vs.known_const || !vs.has_const || !vs.known_bool || !vs.integer_range) {
    free(vs.regs); 
    free(vs.known_func);
    free(vs.d_regs);
    free(vs.slot_type);
  ...
  const jit_inline_ext_t inline_ext = {
    ...
    .shape_transition_proto = shape_transition_proto,
    .imp_shape_transition = imp_shape_transition,
  ...
  MIR_reg_t r_iter_buf = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "iter_buf");
  ...
  MIR_reg_t cached_index_key = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_index_key");
  MIR_reg_t cached_index_value = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_index_value");
  mir_load_imm(ctx, jit_func, cached_index_key, js_mkundef());
  mir_load_imm(ctx, jit_func, cached_index_value, 0);
  MIR_reg_t cached_element_object = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_element_object");
  MIR_reg_t cached_element_key = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_element_key");
  MIR_reg_t cached_element_value = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_element_value");
  MIR_reg_t cached_element_valid = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, "cached_element_valid");
  mir_load_imm(ctx, jit_func, cached_element_valid, 0);
  ...
  jit_integer_range_t *entry_integer_ranges = calloc((size_t)n_locals, sizeof(jit_integer_range_t));
  MIR_reg_t *entry_integer_regs = calloc((size_t)n_locals, sizeof(MIR_reg_t));
  if (entry_integer_ranges && entry_integer_regs && dnum_locals) {
    jit_entry_integer_ranges(func, entry_integer_ranges, n_locals, param_count);
    for (int i = 0; i < n_locals; i++) {
      if (!dnum_locals[i]) continue;
      if (!entry_integer_ranges[i].known) continue;
      char name[48];
      snprintf(name, sizeof(name), "entry_integer_%d", i);
      entry_integer_regs[i] = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, name);
    }
  }
  ...
  const jit_bailout_emit_t bailout_ctx = {
    .val = r_bailout_val,
    .off = r_bailout_off,
    .sp = r_bailout_sp,
    .tramp = bailout_tramp,
    ...
    .local_d_regs = local_d_regs,
    .dnum_locals = dnum_locals,
    .n_locals = n_locals,
    .lbuf = r_lbuf,
    .d_slot = r_d_slot,
  };
    ...
    for (int i = 0; i < param_count && i < JIT_PARAM_HOIST_CAP; i++) {
      ...
      MIR_append_insn(ctx, jit_func,
        MIR_new_insn(ctx, MIR_UBGT,
          ...
          MIR_new_reg_op(ctx, r_argc),
          MIR_new_int_op(ctx, (int64_t)i)));
      mir_load_imm(ctx, jit_func, param_cache[i], mkval(kTypeUndefined, 0));
      MIR_append_insn(ctx, jit_func,
        MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, done)));
  ...
  if (osr_map.count > 0) {
    ...
    if (feat.needs_iter_roots && vs.max > 0) {
      MIR_reg_t r_osr_vstack = MIR_new_func_reg(
        ctx, jit_func->u.func, MIR_T_I64, "osr_vstack");
      MIR_reg_t r_osr_vstack_sp = MIR_new_func_reg(
        ctx, jit_func->u.func, MIR_T_I64, "osr_vstack_sp");
      MIR_append_insn(ctx, jit_func,
        MIR_new_insn(ctx, MIR_MOV,
          MIR_new_reg_op(ctx, r_osr_vstack),
          MIR_new_mem_op(ctx, MIR_T_P,
            osr_base + (MIR_disp_t)offsetof(sv_jit_osr_t, vstack),
            r_vm, 0, 1)));
      MIR_append_insn(ctx, jit_func,
        MIR_new_insn(ctx, MIR_MOV,
          MIR_new_reg_op(ctx, r_osr_vstack_sp),
          MIR_new_mem_op(ctx, MIR_T_I32,
            osr_base + (MIR_disp_t)offsetof(sv_jit_osr_t, vstack_sp),
            r_vm, 0, 1)));
      for (int i = 0; i < vs.max; i++) {
        MIR_label_t slot_done = MIR_new_label(ctx);
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_UBLE,
            MIR_new_label_op(ctx, slot_done),
            MIR_new_reg_op(ctx, r_osr_vstack_sp),
        ...
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_MOV,
            MIR_new_mem_op(ctx, MIR_T_I64,
              ...
              r_iter_roots, 0, 1),
            MIR_new_reg_op(ctx, vs.regs[i])));
        MIR_append_insn(ctx, jit_func, slot_done);
      }
    }
    ...
    if (known_type_locals && local_d_regs) {
      ...
      for (int i = 0; i < n_locals; i++)
        if (known_type_locals[i] == SV_TI_NUM) {
          ...
          if (entry_integer_regs && entry_integer_regs[i]) {
            jit_integer_range_t range = entry_integer_ranges[i];
            MIR_reg_t integer = mir_emit_exact_integer_guard(
              ctx, jit_func, local_regs[i], local_d_regs[i], true, r_d_slot,
              (double)range.min, (double)range.max, osr_type_bail, -i - 1);
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, entry_integer_regs[i]), MIR_new_reg_op(ctx, integer)));
          }
        }
      if (osr_any_num) {
        MIR_append_insn(ctx, jit_func,
...
#define JIT_TRY_MAX 16
  ...
  mir_load_imm(ctx, jit_func, r_result, mkval(kTypeUndefined, 0));

  uint8_t *ip  = func->code;
  uint8_t *end = func->code + func->code_len;

  MIR_reg_t *integer_locals = calloc((size_t)n_locals, sizeof(MIR_reg_t));
  jit_integer_range_t *integer_local_ranges = calloc((size_t)n_locals, sizeof(jit_integer_range_t));
  MIR_reg_t local_reg_limit = 0;
  for (int i = 0; i < n_locals; i++) {
    if (local_regs[i] > local_reg_limit) local_reg_limit = local_regs[i];
    if (local_d_regs[i] > local_reg_limit) local_reg_limit = local_d_regs[i];
  }
  int *local_by_reg = calloc((size_t)local_reg_limit + 1, sizeof(int));
  if (local_by_reg) {
    for (int i = 0; i < n_locals; i++) {
      local_by_reg[local_regs[i]] = i + 1;
      local_by_reg[local_d_regs[i]] = i + 1;
    }
  } else {
    free(integer_locals);
    integer_locals = NULL;
  }
  int integer_local_site = 0;
  bool element_available = false;
...
#define JIT_EMIT_THROW_IF_ERROR(value_reg) do {                       \
  ...
  while (ip < end) {
      ...
      if (lm.entries[i].bc_off == bc_off) {
        ...
        if (integer_locals) {
          for (int li = 0; li < n_locals; li++)
            if (!entry_integer_regs || !entry_integer_regs[li]) integer_locals[li] = 0;
        }
    ...
    MIR_insn_t previous_insn = DLIST_TAIL(MIR_insn_t, jit_func->u.func->insns);
    int integer_store = -1;
    MIR_reg_t integer_value = 0;
    jit_integer_range_t integer_range = {0};
    if (integer_locals && integer_local_ranges && (op == OP_PUT_LOCAL || op == OP_PUT_LOCAL8 ||
        op == OP_SET_LOCAL || op == OP_SET_LOCAL8) && vs.sp > 0 &&
        vs.slot_type[vs.sp - 1] == SLOT_I32) {
      int idx = (op == OP_PUT_LOCAL8 || op == OP_SET_LOCAL8)
        ? sv_get_u8(ip + 1) : sv_get_u16(ip + 1);
      if (idx < n_locals && dnum_locals && dnum_locals[idx]) {
        char name[48];
        snprintf(name, sizeof(name), "integer_local_%d", integer_local_site++);
        integer_value = entry_integer_regs && entry_integer_regs[idx]
          ? entry_integer_regs[idx] : MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, name);
        MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
          MIR_new_reg_op(ctx, integer_value), MIR_new_reg_op(ctx, vstack_top(&vs))));
        integer_store = idx;
        integer_range = entry_integer_regs && entry_integer_regs[idx]
          ? entry_integer_ranges[idx] : vs.integer_range[vs.sp - 1];
      }
    }
    ...
    switch (op) {
      ...
      case OP_CONST_I8: {
        ...
        jit_emit_integer_constant(ctx, jit_func, &vs, dst, d);
        break;
      }
      ...
      case OP_CONST: {
        ...
        if (jit_const_is_heap(cv))
          ...
          if (vtype(cv) == kTypeNumber) {
            ...
            jit_emit_integer_constant(ctx, jit_func, &vs, dst, u.d);
          }
        }
        break;
      }
      ...
      case OP_UNDEF:
        mir_load_imm(ctx, jit_func, vstack_push_const(&vs, mkval(kTypeUndefined, 0)), mkval(kTypeUndefined, 0));
        break;
      case OP_NULL:
        mir_load_imm(ctx, jit_func, vstack_push_const(&vs, mkval(kTypeNull, 0)), mkval(kTypeNull, 0));
        break;
      case OP_TRUE:
      ...
      case OP_GET_LOCAL: {
        ...
        if (integer_locals && integer_locals[idx]) {
          MIR_reg_t dst = vstack_push(&vs);
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, dst), MIR_new_reg_op(ctx, integer_locals[idx])));
          vs.slot_type[vs.sp - 1] = SLOT_I32;
          vs.integer_range[vs.sp - 1] = integer_local_ranges[idx];
          break;
        }
      ...
      case OP_PUT_LOCAL: {
        ...
        bool src_is_num = vstack_prepare_num(
          &vs, vs.sp - 1, ctx, jit_func, r_d_slot
        );
      ...
      case OP_ADD:
      case OP_ADD_NUM: {
        if (jit_emit_integer_arithmetic(ctx, jit_func, &vs, op)) break;
        ...
        bool l_is_num = vstack_prepare_num(
          &vs, vs.sp - 2, ctx, jit_func, r_d_slot
        );
        ...
        if (fb_str_only) {
          vstack_rebox_binop_operands(
            &vs, ctx, jit_func, l_is_num, r_is_num, r_d_slot);
          MIR_label_t slow = MIR_new_label(ctx);
          MIR_label_t done = MIR_new_label(ctx);
          mir_emit_string_concat_fastpath(
          ...
          mir_emit_bailout_check_typed(ctx, jit_func, rd,
            r_bailout_val, bc_off, vs.sp + 1, &bailout_ctx,
          ...
          mir_emit_bailout_jump_typed(ctx, jit_func, bc_off, pre_op_sp,
            &bailout_ctx, vs.sp - 1, l_is_num, vs.sp, r_is_num);
      ...
      case OP_NEG: {
        ...
        bool input_is_num = vstack_prepare_num(
          &vs, top_idx, ctx, jit_func, r_d_slot
        );
        ...
        for (int i = 0; i < vs.sp; i++) {
          mir_emit_slot_boxed(
            ctx, jit_func, vs.regs[i], vs.d_regs[i],
            vs.slot_type ? vs.slot_type[i] : SLOT_BOXED, r_d_slot
          );
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_mem_op(ctx, MIR_T_I64,
        ...
        mir_emit_dnum_rebox(ctx, jit_func, &bailout_ctx);
        for (int i = 0; i < n_locals; i++)
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_mem_op(ctx, MIR_T_I64,
      ...
      case OP_DEC: {
        ...
        if (!input_is_num) {
          ...
          mir_emit_bailout_jump_typed(ctx, jit_func, bc_off, vs.sp,
            &bailout_ctx, top_idx, false, -1, false);
          MIR_append_insn(ctx, jit_func, done);
        }
        break;
      }
      ...
      case OP_POST_DEC: {
        int old_idx = vs.sp - 1;
        bool input_is_num = vstack_prepare_num(
          &vs, old_idx, ctx, jit_func, r_d_slot
        );
        MIR_reg_t rold = vstack_top(&vs);
        vstack_push(&vs);
        int new_idx = vs.sp - 1;
        ...
        if (!input_is_num) {
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, done)));
          MIR_append_insn(ctx, jit_func, bailout);
          mir_emit_bailout_jump_typed(ctx, jit_func, bc_off, vs.sp - 1,
            &bailout_ctx, old_idx, false, -1, false);
          MIR_append_insn(ctx, jit_func, done);
        }
        break;
      }
      ...
      case OP_IS_NULL: {
        vstack_ensure_boxed(&vs, vs.sp - 1, ctx, jit_func, r_d_slot);
        MIR_reg_t rs = vstack_top(&vs);
        uint64_t cmp_val = (op == OP_IS_UNDEF) ? mkval(kTypeUndefined, 0) : mkval(kTypeNull, 0);
        MIR_label_t is_true = MIR_new_label(ctx);
        MIR_label_t is_done = MIR_new_label(ctx);
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_BEQ,
      ...
      case OP_CALL: {
          ...
          if (inline_callee && jit_inlineable(inline_callee)) {
            ...
            for (int i = 0; i < (int)call_argc; i++) {
              inl_arg_num[i] = vstack_prepare_num(
                &vs, inl_arg_base + i, ctx, jit_func, r_d_slot
              );
            ...
            if (speculative) {
              r_spec_guard = MIR_new_func_reg(
                ctx, jit_func->u.func, MIR_T_I64, inl_guard_rn);
              MIR_append_insn(ctx, jit_func,
                MIR_new_insn(ctx, MIR_URSH,
                  MIR_new_reg_op(ctx, r_spec_guard),
            ...
            }
            {
              ...
              mir_emit_decode_ref(ctx, jit_func, r_inl_cl, r_inl_callee);
            }
            {
              ...
              r_inl_super = MIR_new_func_reg(ctx, jit_func->u.func,
                                             MIR_JSVAL, sup_rn);
              mir_load_imm(ctx, jit_func, r_inl_new_target, mkval(kTypeUndefined, 0));
              MIR_append_insn(ctx, jit_func,
                MIR_new_insn(ctx, MIR_MOV,
                  MIR_new_reg_op(ctx, r_inl_super),
                  MIR_new_mem_op(ctx, MIR_T_I64,
            ...
            if (inl_uses_this)
              ...
              mir_load_imm(ctx, jit_func, r_inl_this, mkval(kTypeUndefined, 0));
            ...
            mir_load_imm(ctx, jit_func, r_slow_this, mkval(kTypeUndefined, 0));
        ...
        mir_load_imm(ctx, jit_func, r_call_this, mkval(kTypeUndefined, 0));
      ...
      case OP_SPECIAL_OBJ: {
        uint8_t which = sv_get_u8(ip + 1);
        MIR_reg_t dst = vstack_push(&vs);
        if (which == 0 && func->is_strict) {
          MIR_append_insn(ctx, jit_func,
            MIR_new_call_insn(ctx, 7,
              MIR_new_ref_op(ctx, strict_arguments_proto),
              MIR_new_ref_op(ctx, imp_strict_arguments),
              MIR_new_reg_op(ctx, dst),
              MIR_new_reg_op(ctx, r_vm),
              MIR_new_reg_op(ctx, r_js),
              MIR_new_reg_op(ctx, r_args),
          ...
          MIR_append_insn(ctx, jit_func,
            MIR_new_call_insn(ctx, 6,
              ...
              MIR_new_reg_op(ctx, r_js),
              MIR_new_int_op(ctx, (int64_t)which)));
        } else mir_load_imm(ctx, jit_func, dst, mkval(kTypeUndefined, 0));
        break;
      }
      ...
      case OP_ADD_LOCAL: {
        ...
        if (fb_num_only && loc_is_num) {
          ...
          MIR_append_insn(ctx, jit_func, bail_direct);
          mir_emit_bailout_jump_typed(ctx, jit_func, bc_off, pre_op_sp,
            &bailout_ctx, -1, false, rhs_idx, rhs_is_num);
          MIR_append_insn(ctx, jit_func, done);
          vstack_pop(&vs);
          break;
        }
        ...
        mir_emit_bailout_check(ctx, jit_func, local_regs[idx],
          r_bailout_val, bc_off, pre_op_sp, &bailout_ctx);
      ...
      case OP_STR_APPEND_LOCAL: {
        ...
        if ((int)slot_idx < param_count) {
          if (!writes_params) {
            ...
            mir_load_imm(ctx, jit_func, r_bailout_val, (uint64_t)SV_JIT_BAILOUT);
            mir_emit_bailout_check(ctx, jit_func, r_bailout_val,
              0, bc_off, pre_op_sp, &bailout_ctx);
            MIR_append_insn(ctx, jit_func, arg_in_range);
          }
      ...
      case OP_TO_PROPKEY: {
        ...
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_BEQ,
            ...
            MIR_new_reg_op(ctx, r_bool),
            MIR_new_int_op(ctx, kTypeString)));
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_BEQ,
            MIR_new_label_op(ctx, is_key),
            MIR_new_reg_op(ctx, r_bool),
            MIR_new_int_op(ctx, kTypeSymbol)));
      ...
      case OP_TO_STRING_DEFER_NUMBER: {
        ...
        MIR_reg_t src = vstack_pop(&vs);
        MIR_reg_t dst = vstack_push(&vs);
        MIR_label_t td_done = MIR_new_label(ctx);
        MIR_label_t td_helper = MIR_new_label(ctx);
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, dst),
            MIR_new_reg_op(ctx, src)));
        ...
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_BNE,
            MIR_new_label_op(ctx, td_helper),
            MIR_new_reg_op(ctx, r_bool),
            MIR_new_int_op(ctx, kTypeString)));
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, td_done)));
        MIR_append_insn(ctx, jit_func, td_helper);
        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 5,
            MIR_new_ref_op(ctx, to_string_proto),
            MIR_new_ref_op(ctx, imp_to_string),
            ...
            MIR_new_reg_op(ctx, r_js),
            MIR_new_reg_op(ctx, src)));
        MIR_append_insn(ctx, jit_func, td_done);
        JIT_EMIT_THROW_IF_ERROR(dst);
        break;
      }
      ...
      case OP_GET_ELEM: {
        ...
        if (!specialize && vs.slot_type[vs.sp - 1] == SLOT_I32) {
          char name[48];
          snprintf(name, sizeof(name), "integer_index_%d", mir_next_reg_site(&reg_site_n));
          integer_index = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, name);
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, integer_index),
            MIR_new_reg_op(ctx, vs.regs[vs.sp - 1])));
        }
        ...
        if (specialize && !obj_is_num) {
          ...
          MIR_label_t bail_direct = MIR_new_label(ctx);
          MIR_label_t done = MIR_new_label(ctx);
          int index_site = mir_next_reg_site(&reg_site_n);
          int element_site = mir_next_reg_site(&reg_site_n);
          MIR_reg_t index = mir_emit_array_index_guard(
            ctx, jit_func, key, vs.d_regs[vs.sp], key_is_num,
            r_d_slot, bail_direct, index_site
          );
          MIR_reg_t loaded = r_err_tmp;
          (void)mir_emit_dense_numeric_element_guard(
            ctx, jit_func, obj, index, loaded, false, bail_direct, element_site
          );
          mir_i64_to_d(
            ctx, jit_func, vs.d_regs[vs.sp - 1], loaded, r_d_slot
          );
          vs.slot_type[vs.sp - 1] = SLOT_NUM;
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, done)));
          ...
          mir_emit_bailout_jump_typed(
            ctx, jit_func, bc_off, vs.sp + 1, &bailout_ctx,
            vs.sp - 1, false, vs.sp, key_is_num
          );
          MIR_append_insn(ctx, jit_func, done);
          break;
        }
        ...
        if (!obj_is_num) {
          MIR_label_t slow = MIR_new_label(ctx);
          element_done = MIR_new_label(ctx);
          MIR_label_t loaded = MIR_new_label(ctx);
          if (element_available) {
            MIR_label_t miss = MIR_new_label(ctx);
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_BEQ,
              MIR_new_label_op(ctx, miss), MIR_new_reg_op(ctx, cached_element_valid), MIR_new_int_op(ctx, 0)));
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_BNE,
              MIR_new_label_op(ctx, miss), MIR_new_reg_op(ctx, obj), MIR_new_reg_op(ctx, cached_element_object)));
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_BNE,
              MIR_new_label_op(ctx, miss), MIR_new_reg_op(ctx, key), MIR_new_reg_op(ctx, cached_element_key)));
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, r_err_tmp), MIR_new_reg_op(ctx, cached_element_value)));
            MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, loaded)));
            MIR_append_insn(ctx, jit_func, miss);
          }
          MIR_reg_t index = mir_emit_known_array_index_guard(
            ctx, jit_func, key, integer_index, slow, r_d_slot,
            mir_next_reg_site(&reg_site_n), cached_index_key, cached_index_value
          );
          (void)mir_emit_dense_numeric_element_guard(
            ctx, jit_func, obj, index, r_err_tmp, false, slow,
            mir_next_reg_site(&reg_site_n)
          );
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, cached_element_object), MIR_new_reg_op(ctx, obj)));
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, cached_element_key), MIR_new_reg_op(ctx, key)));
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, cached_element_value), MIR_new_reg_op(ctx, r_err_tmp)));
          mir_load_imm(ctx, jit_func, cached_element_valid, 1);
          MIR_append_insn(ctx, jit_func, loaded);
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, dst), MIR_new_reg_op(ctx, r_err_tmp)));
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, element_done)));
          MIR_append_insn(ctx, jit_func, slow);
          mir_load_imm(ctx, jit_func, cached_element_valid, 0);
        }
        element_available = !obj_is_num;
        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 9,
            ...
            MIR_new_int_op(ctx, (int64_t)bc_off)));
        JIT_EMIT_THROW_IF_ERROR(dst);
        if (element_done) MIR_append_insn(ctx, jit_func, element_done);
        break;
      }
      ...
      case OP_PUT_ELEM: {
        ...
        bool specialize = sv_tfb_specialization_ready(feedback);
        MIR_reg_t integer_index = 0;
        if (!specialize && vs.slot_type[vs.sp - 2] == SLOT_I32) {
          char name[48];
          snprintf(name, sizeof(name), "integer_index_%d", mir_next_reg_site(&reg_site_n));
          integer_index = MIR_new_func_reg(ctx, jit_func->u.func, MIR_T_I64, name);
          MIR_append_insn(ctx, jit_func, MIR_new_insn(ctx, MIR_MOV,
        ...
        bool obj_is_num = vstack_prepare_num(
          &vs, vs.sp - 3, ctx, jit_func, r_d_slot
        );
        ...
        if (specialize && !obj_is_num) {
          ...
          MIR_reg_t index = mir_emit_array_index_guard(
            ctx, jit_func, key, vs.d_regs[vs.sp + 1], key_is_num,
            r_d_slot, bail_direct, index_site
          );
          if (val_is_num)
            mir_d_to_i64(
              ctx, jit_func, val, vs.d_regs[vs.sp + 2], r_d_slot
            );
          else mir_emit_is_num_guard(ctx, jit_func, r_bool, val, bail_direct);

          MIR_reg_t old_value = r_err_tmp;
          MIR_reg_t data = mir_emit_dense_numeric_element_guard(
            ctx, jit_func, obj, index, old_value,
            true, bail_direct, element_site
          );
          MIR_label_t tagged_old_value = NULL;
          MIR_label_t store = NULL;
          if (tagged_old_possible) {
            tagged_old_value = MIR_new_label(ctx);
            store = MIR_new_label(ctx);
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_UBGT,
                MIR_new_label_op(ctx, tagged_old_value),
                MIR_new_reg_op(ctx, old_value),
                MIR_new_uint_op(ctx, NANBOX_PREFIX)));
            MIR_append_insn(ctx, jit_func, store);
          } else {
            mir_emit_is_num_guard(
              ctx, jit_func, r_bool, old_value, bail_direct
            );
          }
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_mem_op(ctx, MIR_JSVAL, 0, data, index,
                sizeof(ant_value_t)),
              MIR_new_reg_op(ctx, val)));
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, done)));
          if (tagged_old_possible) {
            MIR_append_insn(ctx, jit_func, tagged_old_value);
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_UBGE,
                MIR_new_label_op(ctx, bail_direct),
                MIR_new_reg_op(ctx, old_value),
          ...
          MIR_append_insn(ctx, jit_func, bail_direct);
          mir_emit_bailout_jump_typed(
            ctx, jit_func, bc_off, vs.sp + 3, &bailout_ctx,
            -1, false, -1, false
          );
          MIR_append_insn(ctx, jit_func, done);
          break;
        ...
        if (!obj_is_num) {
          ...
          mir_emit_is_num_guard(ctx, jit_func, r_bool, val, slow);
          MIR_reg_t data = mir_emit_dense_numeric_element_guard(
            ctx, jit_func, obj, index, r_err_tmp, true, slow,
            mir_next_reg_site(&reg_site_n)
          );
          mir_emit_is_num_guard(ctx, jit_func, r_bool, r_err_tmp, slow);
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_mem_op(ctx, MIR_JSVAL, 0, data, index, sizeof(ant_value_t)),
              MIR_new_reg_op(ctx, val)));
      ...
      case OP_ITER_NEXT: {
        ...
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_ADD,
            MIR_new_reg_op(ctx, r_iter_buf),
            MIR_new_reg_op(ctx, r_iter_roots),
            MIR_new_int_op(ctx,
              (int64_t)iter_base * (int64_t)sizeof(ant_value_t))));
        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 7,
            MIR_new_ref_op(ctx, iter_next_proto),
            MIR_new_ref_op(ctx, imp_iter_next),
            MIR_new_reg_op(ctx, r_err_tmp),
            MIR_new_reg_op(ctx, r_vm),
            MIR_new_reg_op(ctx, r_js),
            MIR_new_reg_op(ctx, r_iter_buf),
            MIR_new_int_op(ctx, (int64_t)sv_get_u8(ip + 1))));
        JIT_EMIT_THROW_IF_ERROR(r_err_tmp);
        vstack_pop(&vs);
        vstack_pop(&vs);
        vstack_pop(&vs);
        for (int i = 0; i < 5; i++) {
          ...
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, dst),
              MIR_new_mem_op(ctx, MIR_T_I64,
                (MIR_disp_t)(i * (int)sizeof(ant_value_t)), r_iter_buf, 0, 1)));
        }
        if (vs.known_bool) vs.known_bool[vs.sp - 1] = true;
        break;
      }
      ...
      case OP_REGEXP: {
        vstack_ensure_boxed(&vs, vs.sp - 1, ctx, jit_func, r_d_slot);
        vstack_ensure_boxed(&vs, vs.sp - 2, ctx, jit_func, r_d_slot);
        MIR_reg_t flags = vstack_pop(&vs);
        MIR_reg_t pattern = vstack_pop(&vs);
        MIR_reg_t dst = vstack_push(&vs);
        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 7,
            MIR_new_ref_op(ctx, regexp_proto),
            MIR_new_ref_op(ctx, imp_regexp),
            MIR_new_reg_op(ctx, dst),
            MIR_new_reg_op(ctx, r_vm),
            MIR_new_reg_op(ctx, r_js),
            MIR_new_reg_op(ctx, pattern),
            MIR_new_reg_op(ctx, flags)));
        break;
      }
      ...
      case OP_INSTANCEOF: {
        ...
        if (has_ic) {
          ...
          mir_emit_ic_obj_epoch_guard(
            ctx, jit_func, r_ic, slow, "inst", bc_off, ic_idx);
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, r_ic_idx_val),
              MIR_new_mem_op(ctx, MIR_T_I32,
        ...
        if (has_ic) {
          ...
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_BNE,
              ...
              MIR_new_reg_op(ctx, r_ic_shape)));
          mir_emit_ic_obj_epoch_guard(
            ctx, jit_func, r_ic, slow, "cip", bc_off, ic_idx);
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_MOV,
              MIR_new_reg_op(ctx, r_ic_idx_val),
              MIR_new_mem_op(ctx, MIR_T_I32,
      ...
      case OP_SEQ: case OP_EQ: case OP_NE: case OP_SNE: {
        uint8_t feedback = sv_func_type_feedback(func)
          ? sv_func_type_feedback(func)[bc_off] : 0;
        bool numeric_only = sv_tfb_specialization_ready(feedback) &&
          (feedback & SV_TFB_CLASS_MASK) == SV_TFB_NUM;
        bool invert = op == OP_NE || op == OP_SNE;
        ...
        if (numeric_only) {
          ...
          if (!left_is_num) {
            mir_emit_is_num_guard(ctx, jit_func, r_bool, rl, eq_slow);
            mir_i64_to_d(ctx, jit_func, vs.d_regs[left_idx], rl, r_d_slot);
          }
          if (!right_is_num) {
            mir_emit_is_num_guard(ctx, jit_func, r_bool, rr, eq_slow);
            mir_i64_to_d(ctx, jit_func, vs.d_regs[right_idx], rr, r_d_slot);
          }
          mir_emit_numeric_equality(
            ctx, jit_func, vs.d_regs[left_idx], vs.d_regs[right_idx],
            dst, r_bool, invert
          );
          if (vs.known_bool) vs.known_bool[vs.sp - 1] = 1;
          MIR_append_insn(ctx, jit_func,
          ...
          MIR_append_insn(ctx, jit_func, eq_slow);
          if (left_is_num)
            mir_d_to_i64(ctx, jit_func, rl, vs.d_regs[left_idx], r_d_slot);
          if (right_is_num)
            mir_d_to_i64(ctx, jit_func, rr, vs.d_regs[right_idx], r_d_slot);
          MIR_item_t helper = op == OP_SEQ ? imp_seq
            : op == OP_EQ ? imp_eq : op == OP_NE ? imp_ne : imp_sne;
          mir_call_helper2(ctx, jit_func, dst,
                           helper2_proto, helper,
        ...
        int right_idx = vs.sp - 1;
        int left_idx = vs.sp - 2;
        uint8_t right_type = vs.slot_type
          ? vs.slot_type[right_idx] : SLOT_BOXED;
        uint8_t left_type = vs.slot_type
          ? vs.slot_type[left_idx] : SLOT_BOXED;
        ...
        if (!known_number_vs_singleton) {
          vstack_ensure_boxed(&vs, right_idx, ctx, jit_func, r_d_slot);
          vstack_ensure_boxed(&vs, left_idx, ctx, jit_func, r_d_slot);
        }
        MIR_reg_t rr = vstack_pop(&vs);
        MIR_reg_t rl = vstack_pop(&vs);
        MIR_reg_t dst = vstack_push(&vs);
        ...
        if (strict && (r_const || l_const)) {
          if (known_number_vs_singleton) mir_load_imm(ctx, jit_func, dst, invert ? js_true : js_false);
          else {
            MIR_reg_t other = r_const ? rl : rr;
            MIR_label_t matched = MIR_new_label(ctx);
            MIR_label_t eq_done = MIR_new_label(ctx);
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_BEQ,
                MIR_new_label_op(ctx, matched),
                MIR_new_reg_op(ctx, other),
                MIR_new_uint_op(ctx, cval)));
            mir_load_imm(ctx, jit_func, dst, invert ? js_true : js_false);
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, eq_done)));
            MIR_append_insn(ctx, jit_func, matched);
            mir_load_imm(ctx, jit_func, dst, invert ? js_false : js_true);
            MIR_append_insn(ctx, jit_func, eq_done);
          }
        } else if (strict) {
          MIR_label_t eq_slow = MIR_new_label(ctx);
          MIR_label_t eq_done = MIR_new_label(ctx);
          mir_emit_strict_tagged_equality(
            ctx, jit_func, rl, rr, dst, r_bool, invert, eq_slow, eq_done
          );
          MIR_append_insn(ctx, jit_func, eq_slow);
          MIR_item_t helper = op == OP_SEQ ? imp_seq : imp_sne;
          mir_call_helper2(ctx, jit_func, dst,
                           helper2_proto, helper,
                           r_vm, r_js, rl, rr);
          MIR_append_insn(ctx, jit_func, eq_done);
      ...
      case OP_SHL:  case OP_SHR: case OP_USHR: {
        ...
        int rr_idx = vs.sp - 1;
        int rl_idx = vs.sp - 2;
        uint8_t l_type = vs.slot_type ? vs.slot_type[rl_idx] : SLOT_BOXED;
        uint8_t r_type = vs.slot_type ? vs.slot_type[rr_idx] : SLOT_BOXED;
        ...
        jit_integer_range_t result_range = jit_word_range(
          op, l_is_i32 ? vs.integer_range[rl_idx] : (jit_integer_range_t){0},
          r_is_i32 ? vs.integer_range[rr_idx] : (jit_integer_range_t){0});
        if (specialize || (l_is_i32 && r_is_i32)) {
          ...
          MIR_label_t bail_direct = MIR_new_label(ctx);
          MIR_label_t done = MIR_new_label(ctx);
          int left_site = mir_next_reg_site(&reg_site_n);
          int right_site = mir_next_reg_site(&reg_site_n);
          int result_site = mir_next_reg_site(&reg_site_n);
          MIR_reg_t left = mir_emit_word32_guard(
            ctx, jit_func, rl, vs.d_regs[vs.sp - 1], l_is_num, l_is_i32,
            r_d_slot, bail_direct, left_site
          );
          MIR_reg_t right = mir_emit_word32_guard(
            ctx, jit_func, rr, vs.d_regs[vs.sp], r_is_num, r_is_i32,
            r_d_slot, bail_direct, right_site
          );
          mir_emit_word32_binary(
            ctx, jit_func, op, left, right, rd, 0, false, result_site
          );
          vs.slot_type[vs.sp - 1] = SLOT_I32;
          vs.integer_range[vs.sp - 1] = result_range;
          MIR_append_insn(ctx, jit_func,
            MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, done)));
          MIR_append_insn(ctx, jit_func, bail_direct);
          mir_emit_bailout_jump_typed(
            ctx, jit_func, bc_off, vs.sp + 1, &bailout_ctx,
            vs.sp - 1, l_type, vs.sp, r_type
          );
          MIR_append_insn(ctx, jit_func, done);
          break;
        }
      ...
      case OP_BNOT: {
        uint8_t feedback = sv_func_type_feedback(func)
          ? sv_func_type_feedback(func)[bc_off] : 0;
        bool specialize = sv_tfb_specialization_ready(feedback);
        uint8_t input_type = vs.slot_type
        ...
        if (specialize) {
          MIR_reg_t value = vstack_top(&vs);
          MIR_label_t bail_direct = MIR_new_label(ctx);
          MIR_label_t done = MIR_new_label(ctx);
          int input_site = mir_next_reg_site(&reg_site_n);
          MIR_reg_t integer = mir_emit_word32_guard(
            ctx, jit_func, value, vs.d_regs[vs.sp - 1],
            input_is_num, input_is_i32,
            r_d_slot, bail_direct, input_site
          );
          MIR_append_insn(ctx, jit_func,
          ...
          MIR_append_insn(ctx, jit_func, bail_direct);
          mir_emit_bailout_jump_typed(
            ctx, jit_func, bc_off, vs.sp, &bailout_ctx,
            vs.sp - 1, input_type, -1, SLOT_BOXED
          );
          MIR_append_insn(ctx, jit_func, done);
          break;
        }
      ...
      case OP_VOID: {
        ...
        if (vs.has_const) { 
          ...
          vs.known_const[vs.sp - 1] = mkval(kTypeUndefined, 0);
        }
        break;
      }
      ...
      case OP_NEW: {
        ...
        JIT_EMIT_THROW_IF_ERROR(r_new_res);
        break;
      }
      ...
      case OP_RE_EXEC_DISCARD: {
        ...
        MIR_reg_t r_arg = vstack_pop(&vs);
        MIR_reg_t r_call_func = vstack_pop(&vs);
        MIR_reg_t r_call_this = vstack_pop(&vs);
        MIR_reg_t r_call_res = op == OP_RE_EXEC_TRUTHY
          ? vstack_push(&vs) : r_err_tmp;

        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 8,
            MIR_new_ref_op(ctx, regexp_exec_truthy_proto),
            MIR_new_ref_op(ctx, imp_regexp_exec_truthy),
            MIR_new_reg_op(ctx, r_call_res),
            MIR_new_reg_op(ctx, r_vm),
            MIR_new_reg_op(ctx, r_js),
            MIR_new_reg_op(ctx, r_call_func),
            MIR_new_reg_op(ctx, r_call_this),
            MIR_new_reg_op(ctx, r_arg)));
      ...
      case OP_CALL_SUPER: {
        ...
        uint16_t call_argc = sv_get_u16(ip + 1);
        if (call_argc > SV_JIT_ARGS_BUF_CAP ||
            vs.sp < (int)call_argc + 3) { ok = false; break; }
        ...
        MIR_reg_t r_super_nt = vstack_pop(&vs);
        MIR_reg_t r_super_fn = vstack_pop(&vs);
        MIR_reg_t r_super_this = vstack_pop(&vs);
        MIR_reg_t r_super_res = vstack_push(&vs);

        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_MOV,
            MIR_new_mem_op(ctx, MIR_T_I64, 0, r_call_out_this, 0, 1),
            MIR_new_reg_op(ctx, r_super_this)));
        MIR_append_insn(ctx, jit_func,
          MIR_new_call_insn(ctx, 12,
            MIR_new_ref_op(ctx, call_method_proto),
            MIR_new_ref_op(ctx, imp_call_method),
            MIR_new_reg_op(ctx, r_super_res),
            MIR_new_reg_op(ctx, r_vm),
            MIR_new_reg_op(ctx, r_js),
            MIR_new_reg_op(ctx, r_super_fn),
            MIR_new_reg_op(ctx, r_super_this),
            MIR_new_reg_op(ctx, r_args_buf),
            MIR_new_int_op(ctx, (int64_t)call_argc),
            MIR_new_reg_op(ctx, r_super_fn),
            MIR_new_reg_op(ctx, r_super_nt),
            MIR_new_reg_op(ctx, r_call_out_this)));
        ...
        JIT_EMIT_THROW_IF_ERROR(r_super_res);
        MIR_append_insn(ctx, jit_func,
          MIR_new_insn(ctx, MIR_MOV,
            MIR_new_reg_op(ctx, r_this_curr),
            MIR_new_mem_op(ctx, MIR_T_I64, 0, r_call_out_this, 0, 1)));
      ...
      case OP_CALL_METHOD: {
        ...
        if (!is_tail) {
          ...
          if (inline_callee && jit_inlineable(inline_callee)) {
            ...
            mir_load_imm(ctx, jit_func, r_inl_nt, mkval(kTypeUndefined, 0));
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_MOV,
            ...
            jit_emit_inline_body(
              ...
              r_bool, &r_d_slot, mcn, &reg_site_n,
          ...
          if (slot_backed) {
            ...
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_UBGT,
                ...
                MIR_new_reg_op(ctx, r_argc),
                MIR_new_int_op(ctx, (int64_t)idx)));
            mir_load_imm(ctx, jit_func, r_cc_value, mkval(kTypeUndefined, 0));
            MIR_append_insn(ctx, jit_func,
              MIR_new_insn(ctx, MIR_JMP, MIR_new_label_op(ctx, arg_done)));
            MIR_append_insn(ctx, jit_func, arg_in_range);
            MIR_append_insn(ctx, jit_func,
    ...
    for (MIR_insn_t insn = DLIST_NEXT(MIR_insn_t, previous_insn); insn;
         insn = DLIST_NEXT(MIR_insn_t, insn)) {
      if (op != OP_GET_ELEM && MIR_call_code_p(insn->code)) element_available = false;
      for (size_t oi = 0; oi < MIR_insn_nops(ctx, insn); oi++) {
        int output = 0;
        (void)MIR_insn_op_mode(ctx, insn, oi, &output);
        if (!output) continue;
        if (insn->ops[oi].mode == MIR_OP_MEM && op != OP_GET_ELEM) {
          MIR_reg_t base = insn->ops[oi].u.mem.base;
          if (base != r_slotbuf && base != r_lbuf && base != r_args_buf && base != r_iter_roots)
            element_available = false;
        }
        if (!integer_locals || insn->ops[oi].mode != MIR_OP_REG) continue;
        MIR_reg_t reg = insn->ops[oi].u.reg;
        if (reg <= local_reg_limit && local_by_reg[reg])
          integer_locals[local_by_reg[reg] - 1] = 0;
      }
    }
    if (integer_locals && integer_store >= 0) {
      integer_locals[integer_store] = integer_value;
      integer_local_ranges[integer_store] = integer_range;
    }
    if (!ok) break;
    ip += sz;
  }
  ...
  if (!ok || vs.sp > 0) {
    JIT_EMIT_EXIT_RET(MIR_new_uint_op(ctx, mkval(kTypeUndefined, 0)));
  }
  ...
  if (needs_bailout) {
    ...
    MIR_append_insn(ctx, jit_func,
      MIR_new_call_insn(ctx, 17,
        ...
        MIR_new_reg_op(ctx, r_closure),
        MIR_new_reg_op(ctx, r_this_curr),
        feat.needs_new_target ? MIR_new_reg_op(ctx, r_new_target)
          : MIR_new_uint_op(ctx, mkval(kTypeUndefined, 0)),
        feat.needs_super ? MIR_new_reg_op(ctx, r_super_val)
          : MIR_new_uint_op(ctx, mkval(kTypeUndefined, 0)),
        MIR_new_reg_op(ctx, r_args),
        MIR_new_reg_op(ctx, r_argc),
        MIR_new_reg_op(ctx, r_args_buf),
...
#undef JIT_EMIT_EXIT_RET
  ...
  free(integer_locals);
  free(local_by_reg);
  free(integer_local_ranges);
  free(entry_integer_ranges);
  free(entry_integer_regs);
  free(dnum_locals);
  free(local_d_regs);
  free(known_func_locals);
  free(known_type_locals);
...
ant_value_t sv_jit_try_osr(
  ...
  if (vtype(frame->callee) == kTypeFunction) closure = js_func_closure(frame->callee);
  else {
