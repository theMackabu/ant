...
void ant_net_send_udp(
  ...
  size_t len = ant_net_emit_ipv4(
    vm, frame, dst_mac, src_ip, dst_ip, ANT_IP_UDP,
...
static void ant_net_handle_dhcp(
  ant_hvf_vm_t *vm,
  const ant_eth_t *eth,
  const ant_udp_t *udp,
...
static void ant_net_handle_dns(
  ant_hvf_vm_t *vm,
  const ant_eth_t *eth,
  const ant_ipv4_t *ip,
  const ant_udp_t *udp,
  ...
  ant_net_send_udp(vm, eth->src, ntohl(ip->dst), ntohl(ip->src), ANT_DNS_PORT, ntohs(udp->src), resp, (uint16_t)resp_len);
}

static void ant_net_handle_icmp(
  ant_hvf_vm_t *vm,
  const ant_eth_t *eth,
  const ant_ipv4_t *ip,
  const unsigned char *payload,
...
static void ant_net_handle_ipv4(ant_hvf_vm_t *vm, const ant_eth_t *eth, const unsigned char *payload, size_t len) {
  ...
  } else if (ip->proto == ANT_IP_ICMP) ant_net_handle_icmp(vm, eth, ip, l4, l4_len);
}
