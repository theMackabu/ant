...
static int ant_kvm_build_dtb(ant_hvf_vm_t *vm) {
  ...
  if ((rc = ant_kvm_fdt_prop_string(&fdt,
                                    "compatible",
                                    vm->gic_version == 2 ? "arm,cortex-a15-gic" : "arm,gic-v3")) != 0) goto out;
  ...
  if (vm->gic_version == 2) {
    uint64_t gic_reg[] = { ANT_HVF_GIC_DIST_BASE, KVM_VGIC_V2_DIST_SIZE,
                           ANT_HVF_GIC_CPU_BASE, KVM_VGIC_V2_CPU_SIZE };
    if ((rc = ant_kvm_fdt_prop_reg64(&fdt, "reg", gic_reg, 4)) != 0) goto out;
  } else {
    uint64_t gic_reg[] = { ANT_HVF_GIC_DIST_BASE, 0x10000,
                           ANT_HVF_GIC_REDIST_BASE, 0x200000 };
    if ((rc = ant_kvm_fdt_prop_reg64(&fdt, "reg", gic_reg, 4)) != 0) goto out;
  }
  if ((rc = ant_kvm_fdt_end(&fdt)) != 0) goto out;
  ...
  memset(dtb, 0, ANT_KVM_AARCH64_DTB_MAX);
...
static int ant_kvm_finalize_gic(ant_hvf_vm_t *vm) {
  return ant_kvm_set_device_attr(vm->gic_fd,
                                 KVM_DEV_ARM_VGIC_GRP_CTRL,
                                 KVM_DEV_ARM_VGIC_CTRL_INIT,
                                 NULL,
                                 "KVM_SET_DEVICE_ATTR(VGIC init)");
}
...
static int ant_kvm_get_one_reg64(ant_hvf_vm_t *vm, uint64_t id, uint64_t *value, const char *name) {
  ...
  int rc = -errno;
  if (vm->verbose) {
    ant_hvf_verbosef(vm, "KVM_GET_ONE_REG(%s) failed: %s", name, strerror(errno));
  }
  return rc;
}
...
static int ant_kvm_init_vcpu(ant_hvf_vm_t *vm) {
  ...
  int rc = ant_kvm_set_one_reg64(vm,
                                 KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE |
                                   KVM_REG_ARM_CORE_REG(regs.pc),
                                 vm->kernel_entry,
                                 "pc");
  if (rc != 0) return rc;
  rc = ant_kvm_set_one_reg64(vm,
                             KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE |
                               KVM_REG_ARM_CORE_REG(regs.regs[0]),
                             ANT_HVF_DTB_BASE,
                             "x0");
  ...
  rc = ant_kvm_set_one_reg64(vm,
                             KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE |
                               KVM_REG_ARM_CORE_REG(regs.pstate),
                             PSR_MODE_EL1h | PSR_F_BIT | PSR_I_BIT | PSR_A_BIT | PSR_D_BIT,
                             "pstate");
  if (rc != 0) return rc;
  rc = ant_kvm_set_one_reg64(vm,
                             KVM_REG_ARM64 | KVM_REG_SIZE_U64 | KVM_REG_ARM_CORE |
                               KVM_REG_ARM_CORE_REG(sp_el1),
                             ANT_HVF_GUEST_BASE + vm->mem_size - ANT_HVF_PAGE_SIZE,
                             "sp_el1");
  if (rc != 0) return rc;
  return 0;
}

static int ant_kvm_set_vcpu_device_attr_int(ant_hvf_vm_t *vm,
                                            uint32_t group,
                                            uint64_t attr_id,
                                            int value,
                                            const char *op) {
...
static int ant_kvm_init_vcpu_timer(ant_hvf_vm_t *vm) {
  int rc = ant_kvm_set_vcpu_device_attr_int(vm,
                                            ...
                                            27,
                                            "KVM_SET_DEVICE_ATTR(vtimer irq)");
  if (rc != 0) {
  ...
  rc = ant_kvm_set_vcpu_device_attr_int(vm,
                                        ...
                                        30,
                                        "KVM_SET_DEVICE_ATTR(ptimer irq)");
  if (rc != 0) return rc;
  return 0;
}
...
static bool ant_kvm_mmio_unsupported(ant_hvf_vm_t *vm,
                                     const char *region,
                                     bool write,
                                     uint64_t addr,
                                     unsigned size,
