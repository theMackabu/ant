...
static bool napi_slot_get_u64(ant_t *js, ant_value_t obj, internal_slot_t slot, uint64_t *out) {
  ant_value_t value = js_get_slot(js, obj, slot);
...
static bool napi_get_typedarray_data(
  ...
  ant_value_t slot = js_get_slot(js, (ant_value_t)value, SLOT_BUFFER);
...
static ant_value_t napi_callback_trampoline(ant_t *js, ant_value_t *args, int nargs) {
  ...
  ant_value_t data_slot = js_get_slot(js, current, SLOT_DATA);
...
static void napi_tsfn_async_cb(uv_async_t *handle) {
  ...
  for (;;) {
    ...
    ant_value_t cb = js_deref(js, tsfn->func_root);
    if (tsfn->call_js_cb) {
...
static ant_value_t napi_dlopen_common(ant_t *js, ant_value_t module_obj, const char *filename) {
  ...
  if (reg_fn) {
    ret = (ant_value_t)reg_fn(env, (napi_value)exports);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_resolve_deferred(
  ...
  ant_value_t promise = js_deref(nenv->js, def->promise_root);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_create_typedarray(
  ...
  ant_value_t slot = js_get_slot(nenv->js, (ant_value_t)arraybuffer, SLOT_BUFFER);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_create_reference(
  ...
  ref->root = js_root(nenv->js, (ant_value_t)value);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_reference_ref(
  ...
  if (r->refcount == 0) js_root_update(nenv->js, r->root, (ant_value_t)r->value);
  r->refcount++;
...
NAPI_EXTERN napi_status NAPI_CDECL napi_get_typedarray_info(
  ...
  if (data) *data = ta->buffer->data + ta->byte_offset;
  if (arraybuffer) *arraybuffer = (napi_value)js_get(nenv->js, (ant_value_t)typedarray, "buffer");
...
NAPI_EXTERN napi_status NAPI_CDECL napi_get_prototype(
  ...
  ant_napi_env_t *nenv = (ant_napi_env_t *)env;
  if (!nenv || !nenv->js || !result) return napi_set_last(env, napi_invalid_arg, "invalid argument");
  *result = (napi_value)js_get_proto(nenv->js, (ant_value_t)object);
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_get_property(
  ...
  *result = (napi_value)js_getprop_fallback(nenv->js, (ant_value_t)object, name);
  free(name);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_get_named_property(
  ...
  *result = (napi_value)js_getprop_fallback(nenv->js, (ant_value_t)object, utf8name);
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_get_element(
  ...
  *result = (napi_value)js_get(nenv->js, (ant_value_t)object, idx);
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_define_properties(
  ...
  for (size_t i = 0; i < property_count; i++) {
    ...
    if (p->method) {
      ...
      if (is_symbol) {
        if (p->getter) js_set_sym_getter_desc(nenv->js, (ant_value_t)object, key, (ant_value_t)getter_fn, flags);
        if (p->setter) js_set_sym_setter_desc(nenv->js, (ant_value_t)object, key, (ant_value_t)setter_fn, flags);
      } else {
      ...
      value = (ant_value_t)p->value;
    }

    if (is_symbol) {
      js_set_sym(nenv->js, (ant_value_t)object, key, value);
    } else {
      js_set(nenv->js, (ant_value_t)object, key_str, value);
      js_set_descriptor(nenv->js, (ant_value_t)object, key_str, key_len, napi_desc_flags(p->attributes));
    }
  }
...
NAPI_EXTERN napi_status NAPI_CDECL napi_is_arraybuffer(
  ...
  ant_value_t slot = js_get_slot(nenv->js, (ant_value_t)value, SLOT_BUFFER);
  if (vtype(slot) != T_NUM) {
    *result = false;
  } else {
...
NAPI_EXTERN napi_status NAPI_CDECL napi_is_dataview(
  ...
  if (!is_object_type((ant_value_t)value)) {
    *result = false;
    ...
    ant_value_t slot = js_get_slot(nenv->js, (ant_value_t)value, SLOT_DATA);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_is_error(
  ...
  if (!is_object_type((ant_value_t)value)) {
    *result = false;
    ...
    ant_value_t et = js_get_slot(nenv->js, (ant_value_t)value, SLOT_ERR_TYPE);
...
NAPI_EXTERN napi_status NAPI_CDECL napi_coerce_to_bool(
  ...
  ant_napi_env_t *nenv = (ant_napi_env_t *)env;
  if (!nenv || !nenv->js || !result) return napi_set_last(env, napi_invalid_arg, "invalid argument");
  *result = (napi_value)js_bool(js_truthy(nenv->js, (ant_value_t)value));
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_coerce_to_number(
  ...
  ant_napi_env_t *nenv = (ant_napi_env_t *)env;
  if (!nenv || !nenv->js || !result) return napi_set_last(env, napi_invalid_arg, "invalid argument");
  *result = (napi_value)js_mknum(js_to_number(nenv->js, (ant_value_t)value));
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_remove_wrap(
  ...
  free(entry);
  js_set_slot(nenv->js, (ant_value_t)js_object, SLOT_NAPI_WRAP_ID, js_mkundef());
  return napi_set_last(env, napi_ok, NULL);
}
...
NAPI_EXTERN napi_status NAPI_CDECL napi_create_threadsafe_function(
  ...
  tsfn->func_root = js_root(nenv->js, (ant_value_t)func);
