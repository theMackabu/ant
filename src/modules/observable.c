...
static jsval_t js_subobs_error(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(observer) == JS_OBJ) {
    ...
    if (js_type(errorMethod) == JS_FUNC) {
      ...
      if (js_type(result) == JS_ERR) {
        fprintf(stderr, "Error in observer.error: %s\n", js_str(js, result));
      }
    }
  }
...
static jsval_t js_subobs_complete(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(observer) == JS_OBJ) {
    ...
    if (js_type(completeMethod) == JS_FUNC) {
      jsval_t result = js_call_with_this(js, completeMethod, observer, NULL, 0);
      if (js_type(result) == JS_ERR) {
        fprintf(stderr, "Error in observer.complete: %s\n", js_str(js, result));
      }
    }
  }
...
static jsval_t js_cleanup_fn(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(unsubscribe) == JS_FUNC) {
...
static jsval_t execute_subscriber(struct js *js, jsval_t subscriber, jsval_t observer) {
  ...
  if (js_type(subscriberResult) == JS_FUNC) return subscriberResult;
...
static jsval_t js_observable_subscribe(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(subscriberResult) == JS_ERR) {
    jsval_t error_args[1] = {subscriberResult};
    jsval_t error_method = js_get(js, subscriptionObserver, "error");
    if (js_type(error_method) == JS_FUNC) {
      js_call_with_this(js, error_method, subscriptionObserver, error_args, 1);
    }
  } else {
    js_set_slot(js, subscription, SLOT_SUBSCRIPTION_CLEANUP, subscriberResult);
  }
  
  if (subscription_closed(js, subscription)) {
    cleanup_subscription(js, subscription);
  }
...
static jsval_t js_of_subscriber(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(complete) == JS_FUNC) {
    js_call_with_this(js, complete, observer, NULL, 0);
  }
...
static jsval_t js_observable_of(struct js *js, jsval_t *args, int nargs) {
  ...
  for (int i = 0; i < nargs; i++) {
    js_arr_push(js, items, args[i]);
  }
  
  jsval_t subscriber = js_mkobj(js);
  js_set_slot(js, subscriber, SLOT_DATA, items);
  js_set_slot(js, subscriber, SLOT_CFUNC, js_mkfun(js_of_subscriber));
  jsval_t subscriber_func = js_obj_to_func(subscriber);
...
static jsval_t js_from_iteration(struct js *js, jsval_t *args, int nargs) {
  ...
  if (js_type(iteratorMethod) != JS_FUNC) {
  ...
  while (true) {
    ...
    if (js_type(subscription) == JS_OBJ && subscription_closed(js, subscription)) {
      ...
      if (js_type(returnMethod) == JS_FUNC) {
        js_call_with_this(js, returnMethod, iterator, NULL, 0);
      }
      return js_mkundef();
    }
  }
...
static jsval_t js_observable_from(struct js *js, jsval_t *args, int nargs) {
  if (nargs < 1) {
    return js_mkerr_typed(js, JS_ERR_TYPE, "Observable.from requires an argument");
  }
  
  jsval_t x = args[0];
  jsval_t observableMethod = js_get(js, x, get_observable_sym_key());
  
  if (js_type(observableMethod) == JS_FUNC) {
    ...
    jsval_t constructor = js_get(js, observable, "constructor");
    jsval_t C = js_get(js, js_glob(js), "Observable");
    
    if (constructor == C) return observable;
    
    jsval_t subscriber = js_mkobj(js);
    js_set_slot(js, subscriber, SLOT_DATA, observable);
    js_set_slot(js, subscriber, SLOT_CFUNC, js_mkfun(js_from_delegating));
    jsval_t subscriber_func = js_obj_to_func(subscriber);
  ...
  jsval_t subscriber = js_mkobj(js);
  js_set_slot(js, subscriber, SLOT_DATA, data);
  js_set_slot(js, subscriber, SLOT_CFUNC, js_mkfun(js_from_iteration));
  jsval_t subscriber_func = js_obj_to_func(subscriber);
