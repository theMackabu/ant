...
static constexpr uint32_t WS_NATIVE_TAG = 0x57534f42u; // WSOB
static constexpr uint32_t WS_DEFAULT_MAX_PAYLOAD_LEN = 16u * 1024u * 1024u;
...
static void websocket_emit_event(websocket_state_t *ws, int idx, ant_value_t event, ant_offset_t count, bool templated) {
  ...
  if (count == 1 && is_callable(listener)) {
    ant_value_t handler = js_get_slot(listener, SLOT_DATA);
...
static ant_value_t websocket_handler_listener(ant_params_t) {
  ant_value_t handler = js_get_slot(js->current_func, SLOT_DATA);
  if (!is_callable(handler)) return js_mkundef();
  return websocket_call(js, handler, js_getthis(js), args, nargs);
}

static ant_value_t websocket_get_handler(websocket_state_t *ws, int idx) {
  ant_value_t listener = ws->handler_listeners[idx];
  return is_callable(listener) ? js_get_slot(listener, SLOT_DATA) : js_mknull();
}
...
static ant_value_t websocket_set_handler(ant_t *js, websocket_state_t *ws, int idx, ant_value_t value) {
  ...
  if (registered) {
    js_set_slot_wb(js, listener, SLOT_DATA, value);
    return js_mkundef();
  }

  GC_ROOT_SAVE(mark, js);
  GC_ROOT_PIN(js, value);
  listener = js_heavy_mkfun(js, websocket_handler_listener, value);
...
#define WS_HANDLER_ACCESSOR(name, idx) \
  static ant_value_t js_websocket_get_##name(ant_params_t) { \
    ...
    return websocket_get_handler(ws, idx); \
  } \
...
static void websocket_emit_close(websocket_state_t *ws, uint16_t code, const char *reason, bool was_clean) {
  ...
  ant_t *js = ws->js;
  ant_value_t event = websocket_make_event(js, js->builtins.close_event_proto, "close");
...
static void websocket_close_client(websocket_state_t *ws, uint16_t code, bool was_clean) {
  if (!ws || ws->client_close_started) return;
  ws->client_close_started = true;
  ws->client_close_code = code;
  ws->client_close_was_clean = was_clean;
...
static void websocket_client_read_cb(uv_stream_t *handle, ssize_t nread, const uv_buf_t *buf) {
  ...
  if (nread < 0) {
    ...
    websocket_sync_state(ws);
    websocket_close_client(ws, nread == UV_EOF ? 1000 : 1006, nread == UV_EOF);
  }
}
...
static ant_value_t js_websocket_ctor(ant_params_t) {
...
static ant_value_t js_websocket_send(ant_params_t) {
  websocket_state_t *ws = websocket_data(js_getthis(js));
...
static ant_value_t js_websocket_close(ant_params_t) {
  websocket_state_t *ws = websocket_data(js_getthis(js));
  ...
  websocket_sync_state(ws);
  if (ws->is_client) {
    websocket_close_client(ws, 1000, true);
...
static ant_value_t js_message_event_ctor(ant_params_t) {
  if (vtype(call_new_target) == kTypeUndefined)
  ...
  ant_value_t event = websocket_make_event(js, js->builtins.message_event_proto, js_str(js, type_val));
...
void init_websocket_module(ant_t *js) {
  ...
  js->builtins.websocket_proto = js_mkobj(js);
...
#undef WS_DEFINE_HANDLER
  ...
  js_set(js, js->builtins.websocket_ctor, "CLOSING", js_mknum(WS_CLOSING));
  js_set(js, js->builtins.websocket_ctor, "CLOSED", js_mknum(WS_CLOSED));
  js_set_global_builtin(js, "WebSocket", js->builtins.websocket_ctor);
