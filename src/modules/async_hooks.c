...
static inline bool async_hooks_is_callable(jsval_t v) {
  uint8_t t = vtype(v);
  return t == T_FUNC || t == T_CFUNC;
}

static jsval_t async_hooks_call_with_tail_args(
  ant_t *js, jsval_t fn, jsval_t this_arg, jsval_t *args, int nargs, int start_idx
) {
  int call_nargs = nargs - start_idx;
  if (call_nargs <= 0) return sv_vm_call(js->vm, js, fn, this_arg, NULL, 0, NULL, false);

  jsval_t *call_args = (jsval_t *)malloc((size_t)call_nargs * sizeof(jsval_t));
  if (!call_args) return js_mkerr(js, "Out of memory");

  for (int i = 0; i < call_nargs; i++) call_args[i] = args[start_idx + i];
  jsval_t result = sv_vm_call(js->vm, js, fn, this_arg, call_args, call_nargs, NULL, false);
  free(call_args);
  return result;
}

static jsval_t async_local_storage_run(ant_t *js, jsval_t *args, int nargs) {
  ...
  jsval_t prev = js_get_slot(js, this_obj, SLOT_DATA);
  js_set_slot(js, this_obj, SLOT_DATA, args[0]);
  jsval_t result = async_hooks_call_with_tail_args(js, args[1], js_mkundef(), args, nargs, 2);
  js_set_slot(js, this_obj, SLOT_DATA, prev);
  return result;
}

static jsval_t async_local_storage_exit(ant_t *js, jsval_t *args, int nargs) {
  ...
  jsval_t result = async_hooks_call_with_tail_args(js, args[0], js_mkundef(), args, nargs, 1);
  js_set_slot(js, this_obj, SLOT_DATA, prev);
  return result;
}

static jsval_t async_local_storage_enterWith(ant_t *js, jsval_t *args, int nargs) {
  jsval_t this_obj = js_getthis(js);
  if (!is_object_type(this_obj)) {
...
static jsval_t async_local_storage_getStore(ant_t *js, jsval_t *args, int nargs) {
...
static jsval_t async_local_storage_disable(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  jsval_t this_obj = js_getthis(js);
...
static jsval_t async_resource_runInAsyncScope(ant_t *js, jsval_t *args, int nargs) {
  if (nargs < 1 || !async_hooks_is_callable(args[0])) {
    return js_mkerr(js, "AsyncResource.runInAsyncScope(fn[, thisArg, ...args]) requires a function");
  }
  jsval_t this_arg = nargs > 1 ? args[1] : js_mkundef();
  return async_hooks_call_with_tail_args(js, args[0], this_arg, args, nargs, 2);
}

static jsval_t async_resource_emitDestroy(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  return js_getthis(js);
}

static jsval_t async_resource_asyncId(ant_t *js, jsval_t *args, int nargs) {
  (void)js; (void)args; (void)nargs;
  return js_mknum(0);
}

static jsval_t async_resource_triggerAsyncId(ant_t *js, jsval_t *args, int nargs) {
  (void)js; (void)args; (void)nargs;
  return js_mknum(0);
}

static jsval_t async_hook_enable(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  return js_getthis(js);
}

static jsval_t async_hook_disable(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  return js_getthis(js);
}

static jsval_t async_hooks_createHook(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  jsval_t hook = js_mkobj(js);
  js_set(js, hook, "enable", js_mkfun(async_hook_enable));
  js_set(js, hook, "disable", js_mkfun(async_hook_disable));
  return hook;
}

static jsval_t async_hooks_executionAsyncId(ant_t *js, jsval_t *args, int nargs) {
  (void)js; (void)args; (void)nargs;
  return js_mknum(1);
}

static jsval_t async_hooks_triggerAsyncId(ant_t *js, jsval_t *args, int nargs) {
  (void)js; (void)args; (void)nargs;
  return js_mknum(0);
}

static jsval_t async_hooks_executionAsyncResource(ant_t *js, jsval_t *args, int nargs) {
  (void)args; (void)nargs;
  return js_mkobj(js);
}
...
jsval_t async_hooks_library(ant_t *js) {
  ...
  jsval_t als_ctor = js_mkobj(js);
  jsval_t als_proto = js_mkobj(js);
  ...
  js_set_slot(js, als_ctor, SLOT_DEFAULT_CTOR, js_true);
  ...
  js_set(js, lib, "AsyncLocalStorage", js_obj_to_func(als_ctor));

  jsval_t resource_ctor = js_mkobj(js);
  jsval_t resource_proto = js_mkobj(js);
  ...
  js_set_slot(js, resource_ctor, SLOT_DEFAULT_CTOR, js_true);
