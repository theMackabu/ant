...
if (
  ...
  throw new Error(
    "usage: bench_promise_resolution.mjs [async iterations] [sync iterations] [rounds] [case]",
  );
}
...
const nativePromise = Promise.resolve(1);
let promiseSink = nativePromise;

async function awaitNativePromise(iterations) {
  let sum = 0;
  for (let i = 0; i < iterations; i++) sum += await nativePromise;
  return sum;
}
...
function resolveWithNativePromise(resolve) {
  resolve(nativePromise);
}

async function resolvingFunctionWithNativePromise(iterations) {
  let sum = 0;
  for (let i = 0; i < iterations; i++) {
    sum += await new Promise(resolveWithNativePromise);
  }
  return sum;
}

console.log(
  "Promise resolution benchmark: async=" +
    asyncIterations +
    " sync=" +
    syncIterations +
    " rounds=" +
...
// Keep the last allocation observable until every benchmark has completed.
