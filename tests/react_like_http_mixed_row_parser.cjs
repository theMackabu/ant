...
function processFullBinaryRow(response, id, tag, buffer, chunk) {
  ...
  switch (tag) {
    case 84: {
      ...
      if (existing && existing.status !== 'pending') existing.reason.enqueueValue(row);
      else response._chunks.set(id, new ReactPromise('fulfilled', row, null));
      return;
    }
    case 120:
    ...
    case 67: {
      const existing = response._chunks.get(id);
      if (existing && existing.status === 'fulfilled') existing.reason.close(row === '' ? '"$undefined"' : row);
      return;
    }
    default: {
      const existing = response._chunks.get(id);
      if (existing) resolveModelChunk(response, existing, row);
      else response._chunks.set(id, createResolvedModelChunk(response, row));
      return;
    }
  }
}
...
server.listen(0, async () => {
  ...
  try {
    ...
    const prelude = parsed._chunks.get(2);
    const iterable = parsed.controllers.get(1);

    console.log(`preludeStatus:${prelude ? prelude.status : 'missing'}`);
    console.log(`preludeValue:${prelude ? prelude.value : 'missing'}`);
    console.log(`controllerType:${typeof (iterable && iterable.controller)}`);
    console.log(`seen:${iterable ? iterable.seen.join(',') : 'missing'}`);
  } catch (error) {
