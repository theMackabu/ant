...
- [ ] Port Response optimizations in isolated ownership-safe slices after
  ...
  - [x] Borrow direct immutable JS string bodies through a rooted Response slot
    ...
    Completed 2026-08-29.
  - [x] Lazy, fallible Headers materialization and direct native server
    serialization were redesigned and ported under the 200k plan. The narrow
    init-shape proof is implemented as a pinned candidate but remains pending
    server A/B before retention.
...
| `include/internal.h`         | **ASCII cache ported; otherwise superseded**       | The lazy one-code-unit ASCII result cache landed with the descriptor-driven string intrinsic. Find-only interning, UTF validity state, fixed-arena evolution, and object-site shape work already exist in newer forms. Do not copy the PR's broad state-layout change.                                                   |
...
| `include/modules/headers.h`  | **Ported with redesign**                          | Fallible pending-header ownership, transfer, materialization, and direct native-view APIs landed with lazy Response and the HTTP parser transfer work.                                                                                                                                                                    |
...
| `include/modules/response.h` | **Ported with redesign**                          | Borrowed string body ownership and lazy pending headers now have explicit storage and fallible materialization contracts. Server call sites serialize the native view or propagate materialization failure.                                                                                                               |
...
| `src/modules/headers.c`   | **Ported with redesign**                             | One-allocation entries, inline first-entry storage, fallible pending data, parser-node transfer, and ByteString-preserving JS/HTTP boundaries landed with focused and wire-level coverage.                                                                                                                                                                                                                |
...
| `src/modules/response.c`  | **Core slices ported; one candidate pending A/B**     | Borrowed string bodies, lazy native Headers, fallible materialization, and direct server serialization landed. Receiver reuse and the side-effect-safe one-property ResponseInit proof are pinned candidates pending server A/B.                                                                                                                                                                           |
| `src/modules/server.c`    | **Old hunk rejected; safe redesign ported**          | The old unchecked fallible API was not copied. Current server paths serialize pending native Headers directly and make materialization/serialization failure explicit before use.                                                                                                                                                                                                                        |
...
| File                          | Verdict                                             | Evidence and action                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `src/silver/compiler.c`       | **String idea ported; rest superseded**              | A single descriptor-driven string-call intrinsic now covers `indexOf` and `substring` while preserving property identity, fallback, optional chaining, cleanup, and argument snapshots. The old widened GET_ELEM/DEFINE/NEW/IMPORT operands remain rejected.                                                                                                                                       |
| `src/silver/engine.c`         | **String dependency ported**                         | Current bytecode dispatch implements the single operation-driven string intrinsic. The PR's widened opcode decoding remains rejected.                                                                                                                                                                                                                                                                   |
| `src/silver/glue.c`           | **String dependency ported; dynamic key rejected**   | Current helpers implement the guarded string intrinsic. The untraced dynamic GET_ELEM key and stale constructor/import helper designs remain rejected.                                                                                                                                                                                                                                                   |
