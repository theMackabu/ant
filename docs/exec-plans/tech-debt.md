...
- Area: `src/gc/` and `src/shapes.c` — process-static collection and shape state
  ...
  - Note (2026-09-21): `g_strings_sweep_epoch` in `src/gc/strings.c` also keys the per-thread string caches in `src/utf8.c` (UTF-16 length, export length) and `src/modules/regex.c` (WTF-8 validity, which gates `PCRE2_NO_UTF_CHECK`) on (address, length, epoch). `js_destroy` bumps it so a later isolate on the same thread cannot hit a stale entry; that covers the sequential case only. Under concurrent isolates the epoch, the caches and the mark tables must move into the isolate-owned context or be serialized with the rest of the collector state. An atomic epoch alone is not sufficient, because a stale regex cache hit passes an unvalidated subject to PCRE2.
...
- Area: compiled-code lifetime — permanent strings and pinned template objects (`src/runtime.c` code arena, `js_mkstr_permanent`, `gc_pin_permanent`)
  - Issue: Compiled functions (`sv_func_t`, bytecode, constants) live in the code arena and are never freed individually. Every compile-time string literal, including tagged-template cooked and raw strings, is allocated by `js_mkstr_permanent` in `js->pool.permanent`, which is only released at isolate teardown. Tagged-template objects (`OP_PUT_CONST`) and JIT object-literal templates (`jit_helper_object_template`) are kept alive with `gc_pin_permanent`, which is sound only because the code that owns them is immortal. Code compiled repeatedly at runtime (`eval`, `new Function`, VM scripts, template engines that compile on the fly, REPL input, hot module reload) therefore grows memory without bound, including pinned objects that every major collection traces. V8 instead keeps template objects alive through the code that owns their site and collects both when that code dies.
  - Impact: Unbounded memory growth for programs that compile code repeatedly. The pins also add major-collection tracing proportional to their count: on a GC-heavy churn benchmark, 16,000 pinned template objects cost about 4-5%, which matches keeping the same objects alive through a plain array. The string part of that cost is removed by the permanent-string marking fast path (`STR_META_PERMANENT`).
  - Proposed fix: Give each compiled unit (script, module, or function tree) an owner the collector can reclaim once no closure, frame, or JIT code references it. Allocate the unit's literal strings in that unit's own arena instead of the global permanent pool, and replace `gc_pin_permanent` for template objects with ownership by the unit: traced from it and freed with it. Keep `STR_META_PERMANENT` meaning "memory owned outside the collector"; it stays valid because the collector still never frees those strings individually, their unit does. The permanent-root vector then only holds truly isolate-lifetime objects.
  - Validation: A loop compiling `new Function`/`eval` sources that contain tagged templates and object literals must reach steady-state RSS. Template identity per call site must hold for as long as the code is alive, and a GC torture pass must cover reclaiming a unit while one of its closures is still on the stack.
  - Owner: theMackabu
  - Status: backlog (2026-09-21); current behavior matches the existing never-freed code design. Related: the cross-isolate code-arena entry above.
