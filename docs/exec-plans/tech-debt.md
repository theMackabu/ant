...
- Area: roots, symbols, registered-module namespaces, and code arenas — cross-isolate value ownership
  - Issue: `src/gc/roots.c` keeps one process-global static-root array with no isolate owner and no unregister path. Initializers register pointers into `js->builtins` and `js->sym`; `gc_visit_roots(js, ...)` later visits every registered slot using whichever isolate is collecting, and destroying an isolate leaves dangling slots. `src/modules/symbol.c` also overwrites process-global well-known-symbol values on each initialization and stores isolate prototype values in a global iterator fast-path table. `src/esm/library.c` caches a registered library's namespace value globally, so the first isolate's namespace is returned to later isolates. Persistent code/source-intern ownership was fixed in `bcfd34c9`; parser scratch remains process-global with nested mark/rewind scopes. Timer prototype registrations were removed from the global root array in the timer-ownership follow-up, but other initializers still register isolate fields there.
  - Impact: A second isolate is unsafe even without concurrent execution: it can mark values from the wrong heap, reuse another isolate's symbols or module namespace, retain dangling root pointers after teardown, or race on shared parser scratch during concurrent compilation.
  - Proposed fix: Remove isolate fields from the process-global root array and mark `builtins`, `mutable_roots`, and `js->sym` only through isolate-owned visitors; reserve static roots for truly process-lifetime values that cannot point into an isolate heap. Move well-known symbol values, iterator prototype dispatch, registered-module namespace caches, and parser scratch ownership onto `ant_t`. Persistent code arenas and source interns are already isolate-owned. Keep only immutable registered-module names and native initializer functions process-global. Add an interleaved C test that creates two isolates, imports builtins, compiles and runs code in both, destroys either one, and then forces GC and continued execution in the survivor.
  - Status: hard blocker for even sequential live isolates

- Area: reactor, libuv loops, async host modules, and teardown
  - Issue: The 2026-08-26 scan finds 104 `uv_default_loop()` call sites across 27 C/header files. `src/reactor.c` runs that process-global loop. Timer, microtask, next-tick, and immediate queues, pending-work queries, GC traversal, and timer callback owners are now isolate-specific; unrelated host pending-work queries remain global. Timer teardown stops/detaches owned handles and discards queued jobs before VM destruction, leaving native-only close callbacks to finish on the shared loop. Fetch, fs, child process, readline, navigator, net, TLS, server, WebSocket, EventSource, abort, zlib, workers, RPC, LMDB, Wasm, N-API, and buffer code also retain process-global active lists, singleton environments, or isolate values. Several GC marker functions traverse those registries without filtering by owner. Some subsystems are already closer to the required model: events, regex, and process state are isolate-owned; cron/sandbox markers filter by `js`; Atomics wait queues are deliberately shared and locked. `js_destroy()` still invokes global cleanup for RPC, LMDB, and buffer, and it has no general per-isolate close-and-drain phase for outstanding libuv handles.
  - Impact: Isolates cannot run event loops concurrently, other host callback ownership remains shared, one collector can mark another isolate's host values, and terminating one worker can close or free resources belonging to another. Freeing an isolate before all of its handle close callbacks finish also creates teardown use-after-free risk.
  ...
  - Status: timer/job ownership step complete; shared-loop and remaining host-module ownership still block multiple live async isolates and concurrent worker parity. See [timer ownership](completed/isolate-timers.md).
