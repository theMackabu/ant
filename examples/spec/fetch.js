...
test('fetch response headers immutable', (() => {
  try {
...
await testRejects('fetch with aborted signal', () =>
  fetch('data:,ignored', { signal: abortedController.signal })
);

await testRejects('fetch unsupported scheme', () =>
  fetch('file:///tmp/ant-fetch-unsupported')
);
